...
class ksym_holder
{
  public:
   ...
   ~ksym_holder()
    ...
    void enum_symbols(syms_cb cb) const;
  protected:
...
void ksym_holder::enum_symbols(syms_cb cb) const
{
  for ( auto s: m_names )
  {
    addr_sym sym { s.first, s.second->addr };
    cb(&sym, s.second->letter);
  }
}
...
void enum_symbols(syms_cb cb)
{
  s_ksyms.enum_symbols(cb);
}
