...
bm_search::bm_search(unsigned char *pattern, DWORD plen)
 : m_pattern(pattern),
   m_plen(plen)
{
  make(pattern, plen);
...
int bm_search::make(unsigned char *pattern, DWORD plen)
{
  DWORD i;
...
int bm_search::set(unsigned char *pattern, DWORD plen)
{
  if ( m_skip != NULL )
  {
    free(m_skip);
...
const unsigned char *bm_search::search(unsigned char *mem, size_t mlen)
{
